...
int8_t queue(queueADT queue, void *in) {
  ...
  // Si la cola está vacía, asignar el primer nodo y también actualizar
  // `current`
  if (queue->first == NULL) {
    queue->first = newNode;
...
tList removeFromList(tList list, compareFunction cmp, void *elem,
                     int8_t *flag) {
  if (list == NULL) {
    *flag = 0;
    return list;
...
uint64_t sizeQ(queueADT queue) { return queue->amount; }

void toBegin(queueADT queue) { queue->iter = queue->first; }

int8_t hasNext(queueADT queue) { return (queue->iter != NULL); }
