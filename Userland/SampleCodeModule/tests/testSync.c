...
uint64_t testSync(uint64_t argc, char *argv[]) { //{n, use_sem, 0}
    ...
    for (i = 0; i < TOTAL_PAIR_PROCESSES; i++) {
        pids[i] = createProcess((uint64_t)myProcessInc,0, 3, argvDec, (char *[]) {"tty", "tty"});
        pids[i + TOTAL_PAIR_PROCESSES] = createProcess((uint64_t)myProcessInc,0,3, argvInc, (char *[]) {"tty", "tty"});
    }
