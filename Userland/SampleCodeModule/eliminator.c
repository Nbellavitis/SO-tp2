...
void title() {
  ...
  callMoveCursorX((WIDTH / 2) -
                  (strlen("PRESS [ENTER] TO CONTINUE") / 2) * 8 * 2);
...
void configuration() {
  ...
  callMoveCursorX((WIDTH / 2) -
                  (strlen("PLAYER 1 (RED) MOVES WITH A-W-S-D") / 2) * 8 * 2);
  print(RED, "PLAYER 1 (RED) MOVES WITH A-W-S-D\n");
  callMoveCursorX((WIDTH / 2) -
                  (strlen("PLAYER 2 (GREEN) MOVES WITH H-U-J-K") / 2) * 8 * 2);
    ...
    if (c != 0) {
      if (c == '\n') {
        ...
        if ((buffer[0] == '1' || buffer[0] == '2' || buffer[0] == '3' ||
          ...
          flagConfig++;
        } else if ((buffer[0] == '1' || buffer[0] == '2') && i == 1 &&
                   flagConfig == 1) {
...
void midGame() {
  ...
  callMoveCursorX((WIDTH / 2) -
                  (strlen("Do you want to continue? [Y/N]") / 2) * 8 * 2);
...
void pcDirChange() {
  ...
  for (int i = 0; i < 4; i++) {
    if ((possibleKeys[i] == 'u' && player2.prevKey != 'j') ||
      ...
      // Check if the new position is within bounds and not colliding with
      // Player 1
      if (newX >= MOVE && newX < WIDTH - MOVE && newY >= MOVE &&
          newY < HEIGHT - MOVE &&
